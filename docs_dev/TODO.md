...
| 삼항 연산자 | `a ? b : c` | `b if a else c` | **없음** | 미구현   |
...
| 기능         | C++              | Python          | C%             | 상태   |
| ------------ | ---------------- | --------------- | -------------- | ------ |
| split/join   | 직접 구현        | `str.split()` | **없음** | 미구현 |
| upper/lower  | `toupper`      | `str.upper()` | **없음** | 미구현 |
| find/replace | `str.find()`   | `str.find()`  | **없음** | 미구현 |
| trim/strip   | (C++20 없음)     | `str.strip()` | **없음** | 미구현 |
| contains     | `str.find()`   | `in` 연산자   | **없음** | 미구현 |
| substring    | `str.substr()` | `s[1:3]`      | **없음** | 미구현 |
